...
export const DialoguePopup = () => {
  const { activeItemId, eraData, closePopup, consultAdvisor, advisorsConsulted } = useGameStore();
  ...
  const handleSend = async () => {
    ...
    // 2. Simulate AI Response (Backend Integration Point)
    setTimeout(() => {
      setHistory(prev => [...prev, { 
        role: 'assistant', 
        content: `(Simulated AI) You asked about "${userMsg}". As a member of the ${advisor.faction}, I believe we must act with caution.` 
      }]);
      setIsTyping(false);
    }, 1000);
  };
  ...
  const handleEndConversation = () => {
    ...
    const { advisorsConsulted } = useGameStore.getState();
    if (!advisorsConsulted.includes(advisor.id)) {
        // Logic to update state handled in store, but we trigger the final "check"
        useGameStore.getState().consultAdvisor(advisor.id);
    }
    closePopup();
  };
